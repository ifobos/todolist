...
const generatedControllers = {
  ...
  init: router => {
    ...
    router.get(baseUrl + "/findByList/:key", authorize([]), generatedControllers.findByList);
  ...
  */
  ...
  /**
  * UserModel.findByList
  *   @description CRUD ACTION findByList
  *   @param Objectid key Id della risorsa List da cercare
  *
  */
  findByList: async (req, res) => {
    try {
      const result = await UserModel.findByList(req.params.key);
      res.json(result);
    } catch (err) {
      const safeErr = ErrorManager.getSafeError(err);
      res.status(safeErr.status).json(safeErr);
